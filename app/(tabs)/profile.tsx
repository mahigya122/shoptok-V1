...
export default function ProfileScreen() {
  ...
  const save = async () => {
    const uid = await getCurrentUserId();
    await supabase.from("user_profiles").upsert({ id: uid, username, full_name: fullName });
  };

  return (
    <View style={{ flex: 1, padding: spacing.md }}>
      <Image source={{ uri: profile?.avatar_url || "https://picsum.photos/200" }} style={{ width: 80, height: 80, borderRadius: 40 }} />
...
const styles = StyleSheet.create({
  ...
  input: { backgroundColor: colors.surface, color: colors.text, borderWidth: 1, borderColor: colors.border, borderRadius: 12, padding: spacing.sm, marginTop: spacing.sm },
  btn: { backgroundColor: colors.primary, borderColor: colors.primary, borderWidth: 1, borderRadius: 12, padding: spacing.sm, marginTop: spacing.md },
